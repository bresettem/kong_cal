class GoalsController < ApplicationController
	...
	def show
		...
		@set_goal = id.goal
		@akc_price = akc_price?
		@num_days_to_collect = id.num_days_to_collect
		@first_day_to_collect = id.first_day_to_collect
		@unclaimed_coins = id.unclaimed_coins
		@num_days = (Time.now.to_date - @user.goals.last.started_on.to_date).to_i
		@errors = Set.new
		buy_tribes = all_tribes(@unclaimed_coins, @set_goal)
		lab_and_higher = lab_plus(@unclaimed_coins, @set_goal)
		starship_and_higher = starship_plus(@unclaimed_coins, @set_goal)
		city_and_higher = city_plus(@unclaimed_coins, @set_goal)
		only_arenaa = only_arena(@unclaimed_coins, @set_goal)
		...
		@show[:all_tribes] = buy_tribes unless buy_tribes.nil?
		@show[:lab_plus] = lab_and_higher unless lab_and_higher.nil?
		@show[:starship_plus] = starship_and_higher unless starship_and_higher.nil?
		@show[:city_plus] = city_and_higher unless city_and_higher.nil?
		@show[:only_arena] = only_arenaa unless only_arenaa.nil?
		@show[:only_mini_lab] = only_mini_lab unless only_mini_lab.nil?
		@show[:only_mini_starship] = only_mini_starship unless only_mini_starship.nil?
		@show[:no_tribes] = no_tribes unless no_tribes.nil?
		@show = @show.sort_by { |k, v| v[0] }
	...
	private
		...
		def set_last_claimed
			claimed = @user.claim.last
			date = if claimed.nil?
				       Time.now.strftime("%Y-%m-%d")
				     else
					     claimed.claimed.strftime("%Y-%m-%d")
			       end
			@last_claimed = date
		end
		...
		def goal_params
			params.require(:goal).permit(:unclaimed_coins, :goal, :started_on, :num_days_to_collect, :first_day_to_collect)
		end
		...
		def no_tribes(unclaimed_coins, goal)
			...
			while goal >= sum
				...
				if goal < sum
					...
					return [index, daily_yield, sum, no_tribes_arr]
				end
				if @num_days_to_collect.nonzero?
					days = Date.today.next_day(index)
					if days >= @first_day_to_collect
						unless index % @num_days_to_collect === 0
							index += 1
							next
						end
					else
		...
		def all_tribes(unclaimed_coins, goal)
			buy_tribes_arr = []
			sum = unclaimed_coins
			mini_starship = false
			lab = false
		...
		def lab_plus(unclaimed_coins, goal)
			buy_tribes_arr = []
			sum = unclaimed_coins
			lab = false
			starship = false
		...
		def starship_plus(unclaimed_coins, goal)
			buy_tribes_arr = []
			sum = unclaimed_coins
			starship = false
			city = false
		...
		def city_plus(unclaimed_coins, goal)
			buy_tribes_arr = []
			sum = unclaimed_coins
			city = false
			arena = false
		...
		def only_arena(unclaimed_coins, goal)
			buy_tribes_arr = []
			sum = unclaimed_coins
			arena = false
			daily_yield = get_sum[0]
